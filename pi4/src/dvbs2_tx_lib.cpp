// aff3ct/dvbs2自体のdvbs2_tx main()をextern "C"関数化しただけの薄いラッパー。
#include <atomic>
#include <fstream>
#include <aff3ct.hpp>
...
// ★dvbs2_rx_lib.cppと同じ理由(FIFOを閉じるだけでは内部のブロッキング実行ループが
// 止まらずCPU/メモリを消費し続ける)で、外部から停止要求できるフラグを追加する。
...
// 遅延する(CPU競合が残る)ことを確認した(Android/iOS版での知見)。まずはシングル
// スレッドで動作確認する(DVBS2_iOS_Port_PoC.md 12.11続報、Pi4ではスレッドピニング
// 制約が無いためMULTI_THREADED再有効化は動作確認後に検討)。
...
// 元は int main(int argc, char** argv)。tx_main.cppからライブラリ関数として呼び出せる
// ように extern "C" 関数へ変更した(名前修飾を避けCブリッジ層から呼べるようにするため)。
...
// 素通しすると terminate() 経由でSIGABRTしプロセスごと即死する。呼び出し側にエラーとして
// 伝播できるよう、関数全体を例外境界で囲む。
try
{
