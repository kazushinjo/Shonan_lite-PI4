...
# ★KMS無効(linuxfb)環境ではconsole=tty1指定によりsystemdの起動ログ
# ("Completed socket interaction for boot stage ..."等)がフレームバッファへ
# 直接書き込まれ、Shonan_Lite/Langstoneの画面に文字が重なって映り込む不具合を
# 実機で確認した。console出力先をtty1からtty3へ変更し、実機LCD(tty1)には
# 何も出力させないようにする。
if [ -f "$CMDLINE_FILE" ] && grep -q 'console=tty1' "$CMDLINE_FILE"; then
  sudo sed -i 's/console=tty1/console=tty3/' "$CMDLINE_FILE"
  echo "${CMDLINE_FILE}のconsole出力先をtty1からtty3へ変更しました(反映には再起動が必要です)。"
fi
...
# ★eglfs(KMS/DRM)はDFR0550のDSIパネルでLangstone(legacy framebuffer)との
# 組み合わせ次第でパネル初期化が競合し、バックライト初期化タイムアウトや
# 画面の色化け・no screens available等で起動できなくなる不具合を実機で
# 確認した。KMSに依存しないlinuxfb(/dev/fb0直描画)に変更し、Langstone側の
# config.txt変更(KMS無効化)と無関係に安定動作するようにする。
...
Conflicts=shonan-gui.service langstone.service
...
# 起動メニューと画面を共有するサービスを停止してからメニューを表示する。
